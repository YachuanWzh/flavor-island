...
跨平台（Windows / macOS）桌面端浮岛（Floating Island）：实时展示 **flavor-code** coding agent 的工作状态。
参考 [CodeIsland](https://github.com/wxtsky/CodeIsland)（macOS 刘海灵动岛）与
[CodeIslandWin](https://github.com/wxtsky/CodeIslandWin)（Windows 移植版）实现，
对 flavor-code 提供**原生支持**。
...
- **问题回答** — 渲染 AskUserQuestion 卡片（单选 / 多选 / 文本）
- **会话概览** — 多个会话排序展示，正在工作的会话置顶
- **拖拽移动** — 拖到任意位置，双击复位到顶部居中
- **点击穿透** — 浮岛周围的透明区域不遮挡桌面
